...
RSpec.describe PublisherUsersPresenter do
  subject { presenter }

  let(:presenter) { described_class.new(user, policy, models) }
  let(:user) { build(:user) }
  let(:policy) { PublisherUsersPolicy.new([SubjectPolicyAgent.new(:User, user), PublisherPolicyAgent.new(publisher), UserPolicyAgent.new(nil)]) }
  ...
  describe 'presenters' do
    ...
    it do
      ...
      subject.each.with_index do |model_presenter, index|
        ...
        expect(model_presenter.policy).to be_a(PublisherUsersPolicy)
