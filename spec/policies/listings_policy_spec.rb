...
RSpec.describe ListingsPolicy, type: :policy do
  ...
  let(:object_agent) { ObjectPolicyAgent.new(:Listing, listing) }
  ...
  context "Authenticated" do
    ...
    VerbPolicyAgent::VERB_TYPES.each do |verb_type|
      ...
      when :Action
        ActionPolicyAgent::ACTIONS.each do |action|
          context "#{action}?" do
            if %i[action create index show].include?(action)
              it { expect(subject.send("#{action}?")).to be true }
            else
              it { expect(subject.send("#{action}?")).to be false }
            end
            context 'Grant' do
              let(:action_agent) { ActionPolicyAgent.new(action) }
              before { PolicyMaker.permit!(subject_agent, action_agent, object_agent) }
              if %i[action create index destroy show update].include?(action)
                it { expect(subject.send("#{action}?")).to be true }
              else
                it { expect(subject.send("#{action}?")).to be false }
              end
  ...
  context "Administrative" do
    ...
    VerbPolicyAgent::VERB_TYPES.each do |verb_type|
      ...
      when :Action
        ActionPolicyAgent::ACTIONS.each do |action|
          context "#{action}?" do
            if %i[action add create destroy index remove show update].include?(action)
              it { expect(subject.send("#{action}?")).to be true }
            else
              it { expect(subject.send("#{action}?")).to be false }
            end
            context 'Grant' do
              let(:action_agent) { ActionPolicyAgent.new(action) }
              before { PolicyMaker.permit!(subject_agent, action_agent, object_agent) }
              if %i[action add create destroy index show remove update].include?(action)
                it { expect(subject.send("#{action}?")).to be true }
              else
                it { expect(subject.send("#{action}?")).to be false }
              end
